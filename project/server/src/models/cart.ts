...
import { findProduct, PRODUCT } from '../utils';
           ...
           if(products !== ''){
            ...
            if(lookedFor != null){
               ...
               if(cartProducts !== ''){
                    ...
                    await fsPromises.writeFile(cartFile,JSON.stringify(cartProductsJSON,null,'\t'))
                    
               }else{
                    cartProductsJSON = [lookedFor];
                    await fsPromises.writeFile(cartFile, JSON.stringify(cartProductsJSON,null,'\t'));
               }
               return { message: "Product added to the cart successfully!", data:lookedFor }
            }else{
                throw { error: ProductNotFound, message: "Wrong id or product doesn't exist..." }
            }
           }else{
            throw { error: NoProducts, message: "No products available to add to the cart.." }
           }
       } catch (e) {
           throw e
       }
   ...
   async deleteProduct (id: string): Promise<{message:string, data:PRODUCT}> {
       try {
           ...
           if(cartProducts !== ''){
              ...
              const lookedFor : PRODUCT | undefined = findProduct(cartProductsJSON, id);
              if(lookedFor != null){
                const arrayID : number = cartProductsJSON.indexOf(lookedFor);
                const [deleted] = cartProductsJSON.splice(arrayID,1);
                await fsPromises.writeFile(productsFile,JSON.stringify(cartProductsJSON,null,'\t'));
                return { message: "Product deleted from the cart successfully", data: deleted }
              }else{
                  throw { error: ProductNotFound, message:"Wrong id or the product isn't at the cart..." }
              }
           }else{
